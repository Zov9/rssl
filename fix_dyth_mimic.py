...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2,scheduler, worst_k,info_pairs,learning_status,N):
    ...
    for item in worst_k:
            if info_pairs is not None and epoch>100 and args.usedyth == 1 :  #add epoch > 100 after checked no bug
                ...
                max_value = torch.max(torch.tensor(list(dict_from_pairs.values())), dim=0).values
                min_value = torch.min(torch.tensor(list(dict_from_pairs.values())), dim=0).values
                ...
                if not math.isnan(dict_from_pairs[item]):
                    dy_threshold[item] = min_value/dict_from_pairs[item]
                    dy_threshold[item] = dy_threshold[item]/(2-dy_threshold[item])
                    dy_threshold[item] = dy_threshold[item] * args.weakth
