...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2):
    ...
    bar.finish()
    '''
    print("type u_target",type(u_target),"type u_target[0]",type(u_target[0]))
    print("len u_target",len(u_target),"len u_target[0]",len(u_target[0]))
    print("type u_pred",type(u_pred),"type u_pred[0]",type(u_pred[0]))

    print("len u_pred",len(u_pred),"len u_pred[0]",len(u_pred[0]))

    print("u_target[0]",u_target[0],"\nu_pred[0]",u_pred[0])
    '''
    sim_confusion_matrix = calculate_confusion_matrix(u_target, u_pred, threshold=0.5)
    with open('cf100 10081911_imb10 scm.txt', 'a') as file:
...
def validate(valloader, model, criterion, mode, epoch):
    ...
    with open('cf100 10081911_imb10.txt', 'a') as file:
        file.write("Epoch: "+str(epoch)+'\n')
    ...
    with open('cf100 10081911_imb10.txt', 'a') as file:
        ...
        for cls, acc in per_class_accuracies.items():
            file.write(f'{cls} Accuracy: {acc:.2f}\n')
