...
txtpath = "/data/lipeng/ABC/txt/cf100_1028_quantitytry1.txt"
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2,learning_status,N):
   ...
   # print("l_cm_pred len ,len[0], len[0][0]",len(l_cm_pred),len(l_cm_pred[0]))
    ...
    with open(txtpath, 'a') as file:
                ...
                file.write(f"Counter: {dict(counter1)}\n")
...
def clsloss(input_tensor,target_tensor,num_class):
    classwise_loss = []
    num_classes = num_class
    for i in range(num_classes):
        class_mask = (target_tensor == i)
        if class_mask.any():
            class_loss = F.cross_entropy(input_tensor[class_mask], target_tensor[class_mask])
