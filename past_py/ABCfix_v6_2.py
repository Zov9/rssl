...
# I want to implement a contra learning here  12111448  here i only test about top k performance k is among 2,3,4,5
...
txtpath = '/data/lipeng/ABC/txt/cf100_1213_top5.txt'
...
def main():
    ...
    if args.resume:
        ...
        logger.set_names(['Train Loss', 'Train Loss X', 'Train Loss U', 'abcloss','Test Loss', 'Test Acc.','Top 2','Top 3','Top 4','Top 5'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        test_loss, test_acc,top2,top3,top4,top5, testclassacc = validate(test_loader, ema_model, criterion, mode='Test Stats ',epoch = epoch)
        if args.dataset == 'cifar10':
        ...
        logger.append([train_loss, train_loss_x, train_loss_u,abcloss, test_loss, test_acc,top2,top3,top4,top5])
...
def validate(valloader, model, criterion, mode, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            prec1, prec2,prec3,prec4,prec5 = accuracy(outputs2, targets, topk=(1,2,3,4,5))
            losses.update(loss.item(), inputs.size(0))
            top1.update(prec1.item(), inputs.size(0))
            top2.update(prec2.item(), inputs.size(0))
            top3.update(prec3.item(), inputs.size(0))
            top4.update(prec4.item(), inputs.size(0))
            ...
            # plot progress
            bar.suffix = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | ' \
                         'Loss: {loss:.4f} | top1: {top1: .4f} | top2: {top2: .4f}  | top3: {top3: .4f}  | top4: {top4: .4f}  | top5: {top5: .4f}'.format(
                batch=batch_idx + 1,
                size=len(valloader),
                data=data_time.avg,
                bt=batch_time.avg,
    ...
    return (losses.avg, top1.avg, top2.avg , top3.avg , top4.avg , top5.avg ,accperclass)
