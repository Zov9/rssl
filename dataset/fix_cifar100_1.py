import numpy as np
from PIL import Image

import torchvision
import torch
from torchvision.transforms import transforms
from RandAugment import RandAugment
from RandAugment.augmentations import CutoutDefault

# Parameters for data
cifar100_mean = (0.5071, 0.4867, 0.4408)  # equals np.mean(train_set.train_data, axis=(0,1,2))/255
cifar100_std = (0.2675, 0.2565, 0.2761)
# Augmentations.
transform_train = transforms.Compose([
    transforms.RandomCrop(32, padding=4),
    transforms.RandomHorizontalFlip(),
    transforms.ToTensor(),
    transforms.Normalize(cifar100_mean, cifar100_std)
])

transform_strong = transforms.Compose([
    transforms.RandomCrop(32, padding=4),
    transforms.RandomHorizontalFlip(),
    transforms.ToTensor(),
    transforms.Normalize(cifar100_mean, cifar100_std)
])
transform_strong.transforms.insert(0, RandAugment(3, 4))
transform_strong.transforms.append(CutoutDefault(16))

#seems to be 32*32  16 means cut one-fourth of the pic

transform_val = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(cifar100_mean, cifar100_std)
])


class TransformTwice:
    def __init__(self, transform, transform2):
        self.transform = transform
        self.transform2 = transform2

    def __call__(self, inp):
        out1 = self.transform(inp)
        out2 = self.transform2(inp)
        out3 = self.transform2(inp)
        return out1, out2, out3
    
class TransformTwice1:
    def __init__(self, transform, transform2, wk=None):
        self.transform = transform
        self.transform2 = transform2
        self.wk = wk

    def update_transforms(self, wk):
        # Update the wk variable
        self.wk = wk
        # Adjust the intensity of RandAugment and CutoutDefault based on wk
        if self.wk is not None:
            # Weaker augmentation parameters
            n = 3  # Example: less number of augmentations
            m = 2  # Example: lower magnitude
            cutout_size = 16  # Example: smaller cutout size
        else:
            # Stronger augmentation parameters
            n = 3
            m = 4
            cutout_size = 16

        # Update the transformations
        self.transform2.transforms[0] = RandAugment(n, m)
        self.transform2.transforms[-1] = CutoutDefault(cutout_size)

    def __call__(self, inp):
        out1 = self.transform(inp)
        out2 = self.transform2(inp)
        out3 = self.transform2(inp)
        return out1, out2, out3


def get_cifar100(root, l_samples, u_samples, transform_train=transform_train, transform_strong=transform_strong,
                transform_val=transform_val, download=True):
    base_dataset = torchvision.datasets.CIFAR100(root, train=True, download=download)
    train_labeled_idxs, train_unlabeled_idxs= train_split(base_dataset.targets, l_samples, u_samples)

    train_labeled_dataset = CIFAR100_labeled(root, train_labeled_idxs, train=True, transform=transform_train,transform_name='transform_train')
    train_unlabeled_dataset = CIFAR100_unlabeled(root, train_unlabeled_idxs, train=True,
                                                transform=TransformTwice(transform_train, transform_strong),transform_name='transformtwice')
    test_dataset = CIFAR100_labeled(root, train=False, transform=transform_val, download=True)

    print (f"#Labeled: {len(train_labeled_idxs)} #Unlabeled: {len(train_unlabeled_idxs)}")
    return train_labeled_dataset, train_unlabeled_dataset, test_dataset


def train_split(labels, n_labeled_per_class, n_unlabeled_per_class):

    labels = np.array(labels)
    train_labeled_idxs = []
    train_unlabeled_idxs = []

    for i in range(100):
        idxs = np.where(labels == i)[0]
        train_labeled_idxs.extend(idxs[:n_labeled_per_class[i]])
        train_unlabeled_idxs.extend(idxs[:n_labeled_per_class[i] + n_unlabeled_per_class[i]])
    return train_labeled_idxs, train_unlabeled_idxs


def update_dataset(obj,twk):
    obj.wk = twk



class CIFAR100_labeled(torchvision.datasets.CIFAR100):

    def __init__(self, root, indexs=None, train=True,
                 transform=None, target_transform=None,
                 download=True,transform_name = None):
        super(CIFAR100_labeled, self).__init__(root, train=train,
                                              transform=transform, target_transform=target_transform,
                                              download=download)
        if indexs is not None:
            self.data = self.data[indexs]
            self.targets = np.array(self.targets)[indexs]
        self.data = [Image.fromarray(img) for img in self.data]
        self.transform_name = transform_name
        self.wk = None

    def __getitem__(self, index):
        """
        Args:
            index (int): Index

        Returns:
            tuple: (image, target) where target is index of the target class.
        """
        img, target = self.data[index], self.targets[index]

        if self.transform is not None:
            img = self.transform(img)
            print(f'Applied transform for labeled: {self.transform_name}')

        if self.target_transform is not None:
            target = self.target_transform(target)
        print('image len',len(img))
        #print('target len',len(target))
        #print('index len',len(index))
        return img, target, index





'''
#下面是一开始的 cifar100_labeled
class CIFAR100_labeled(torchvision.datasets.CIFAR100):

    def __init__(self, root, indexs=None, train=True,
                 transform=None, target_transform=None,
                 download=True,transform_name = None):
        super(CIFAR100_labeled, self).__init__(root, train=train,
                                              transform=transform, target_transform=target_transform,
                                              download=download)
        if indexs is not None:
            self.data = self.data[indexs]
            self.targets = np.array(self.targets)[indexs]
        self.data = [Image.fromarray(img) for img in self.data]
        self.transform_name = transform_name

    def __getitem__(self, index):
        """
        Args:
            index (int): Index

        Returns:
            tuple: (image, target) where target is index of the target class.
        """
        img, target = self.data[index], self.targets[index]

        if self.transform is not None:
            img = self.transform(img)
            print(f'Applied transform for labeled: {self.transform_name}')

        if self.target_transform is not None:
            target = self.target_transform(target)
        print('image len',len(img))
        #print('target len',len(target))
        #print('index len',len(index))
        return img, target, index
'''






'''

class CIFAR100_unlabeled(CIFAR100_labeled):

    def __init__(self, root, indexs, train=True,
                 transform=None, target_transform=None,
                 download=True):
        super(CIFAR100_unlabeled, self).__init__(root, indexs, train=train,
                                                transform=transform, target_transform=target_transform,
                                                download=download)
        #self.targets = np.array([-1 for i in range(len(self.targets))])
        self.indices = indexs
'''


class CIFAR100_unlabeled(torchvision.datasets.CIFAR100):

    def __init__(self, root, indexs, train=True,
                 transform=None, target_transform=None,
                 download=True):
        super(CIFAR100_unlabeled, self).__init__(root, train=train,
                                                 transform=transform, target_transform=target_transform,
                                                 download=download)
        if indexs is not None:
            self.data = self.data[indexs]
            self.targets = np.array(self.targets)[indexs]
        self.data = [Image.fromarray(img) for img in self.data]
        #self.targets = np.array([-1 for _ in range(len(self.targets))])  # Assign a dummy label
        self.indices = indexs

    def __getitem__(self, index):
        img, target = self.data[index], self.targets[index]

        if self.transform is not None:
            img = self.transform(img)

        if self.target_transform is not None:
            target = self.target_transform(target)

        return img, target, index


#下面是没完成的 一开始想对无标签样本做不同强度的数据增强 下面是没完成的 后面先考虑对有标签样本做不同强度的数据增强
'''
class CIFAR100_unlabeled(torchvision.datasets.CIFAR100):

    def __init__(self, root, indexs, train=True,
                 transform=None, target_transform=None,
                 download=True,transform_name = None):
        super(CIFAR100_unlabeled, self).__init__(root, train=train,
                                                 transform=transform, target_transform=target_transform,
                                                 download=download)
        if indexs is not None:
            self.data = self.data[indexs]
            self.targets = np.array(self.targets)[indexs]
        self.data = [Image.fromarray(img) for img in self.data]
        #self.targets = np.array([-1 for _ in range(len(self.targets))])  # Assign a dummy label
        self.indices = indexs
        self.transform_name = transform_name
        self.wk = None
    

    def __getitem__(self, index):
        img, target = self.data[index], self.targets[index]

        if self.transform is not None:
            img = self.transform(img)
            print(f'Applied transform for unlabeled: {self.transform_name}')

        if self.target_transform is not None:
            target = self.target_transform(target)
        print('image len unlable',len(img))

        return img, target, index
'''