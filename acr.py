...
from utils_acr import AverageMeter, accuracy
from utils_acr import Logger
...
def main():
    ...
    parser.add_argument('--out1', default='result',
                        help='Directory to output the result')
    ...
    train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
          model, optimizer, ema_model, scheduler,txtpath,info_pairs,worst_k,twk)
    args.logger.close()
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        cls_rep_upsu_part =  {i: [] for i in range(args.num_classes)}
        cls_center_final = {}
        cls_rep_final = {i: [] for i in range(args.num_classes)}
        ...
        for batch_idx in range(args.eval_step):
            ...
            if epoch <= 100 :
                selected_thresholds = dy_threshold[targets_u_h2]
                
            else:
                selected_thresholds = dy_threshold[targets_u_b]
            ...
            '''
            mask = max_probs.ge(selected_thresholds)
            mask_h2 = max_probs_h2.ge(selected_thresholds)
            mask_b = max_probs_b.ge(selected_thresholds)
            mask_t = max_probs_t.ge(selected_thresholds)
            ...
            int_mask = torch.cat([int_mask, int_mask], dim=0).float()  
            org_mask = mask_ss_b_h2
            ...
            Lx = (F.cross_entropy(logits_x, targets_x, reduction='none')*int_mask1).mean()
            #Lx_b = F.cross_entropy(logits_x_b + args.adjustment_l2, targets_x, reduction='mean')
            Lx_b = (F.cross_entropy(logits_x_b + args.adjustment_l2, targets_x, reduction='none')*int_mask1).mean()
            Lu = (F.cross_entropy(logits_u_s_twice, targets_u_twice,
                                  reduction='none') * int_mask * mask_twice_ss_t).mean()
            Lu_b = (F.cross_entropy(logits_u_s_b_twice, targets_u_h2_twice,
                                    reduction='none') * int_mask * mask_twice_ss_b_h2).mean()
        ...
        if args.local_rank in [-1, 0]:
            test_loss, test_acc, test_top5_acc, test_acc_b, test_top5_acc_b,twk = test(args, test_loader, test_model, epoch,txtpath,args.wk)
...
def test(args, test_loader, model, epoch,txtpath,wk):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            unbiasedscore = F.softmax(outputs_b)
    ...
    return losses.avg, top1.avg, top5.avg, top1_b.avg, top5_b.avg, tuple(smallest_classes_indices)
